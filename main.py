...
# Public URL for this separate supervisor Render app.
# You can override this in Render with APP_BASE_URL if the service URL changes.
APP_BASE_URL = os.getenv("APP_BASE_URL", "https://supervisor-mn0f.onrender.com").rstrip("/")

# These default to the supervisor app URL unless you explicitly override them in Render.
WEBEX_REDIRECT_URI = os.getenv("WEBEX_REDIRECT_URI", f"{APP_BASE_URL}/oauth/callback")
WEBEX_WEBHOOK_TARGET_URL = os.getenv("WEBEX_WEBHOOK_TARGET_URL", f"{APP_BASE_URL}/webex/calling-events")
...
def health():
    return {
        ...
        "webhook_target_url": WEBEX_WEBHOOK_TARGET_URL,
    }
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .supervisor-action-btn.whisper {
      background: #e0f2fe;
      color: #075985;
    }

    .supervisor-action-btn.barge {
      background: #fee2e2;
      color: #991b1b;
    }

    .supervisor-action-btn:disabled {
      background: #e5e7eb;
      color: #9ca3af;
      cursor: not-allowed;
    }
    ...
    .action-modal h2 { margin: 0 0 8px; }
    .action-modal p { color: #475569; line-height: 1.5; }
...
<main>
  ...
  <div id="supervisorActionModal" class="action-modal-backdrop">
    <div class="action-modal">
      <h2 id="supervisorActionTitle">Supervisor Action</h2>
      <p id="supervisorActionText"></p>
      <div id="supervisorActionDialCode" class="action-dial-code"></div>
      <div class="action-modal-actions">
        <a id="supervisorActionDialLink" class="button" href="#">Open Dialer</a>
        <button onclick="copySupervisorActionCode()">Copy Code</button>
        <button class="secondary" onclick="closeSupervisorActionModal()">Close</button>
      </div>
    </div>
  </div>
...
const DEFAULT_COLUMNS = [
  ...
  { key: "whisper", label: "Whisper", render: a => renderSupervisorActionButton(a, "whisper") },
  { key: "barge", label: "Barge", render: a => renderSupervisorActionButton(a, "barge") }
];
...
let currentSupervisorActionCode = "";

function renderSupervisorActionButton(agent, action) {
  ...
  const cssClass = action === "whisper" ? "whisper" : "barge";

  let title = `${label} this call`;
  if (!hasExtension) {
    title = `${label} unavailable: no extension found`;
  } else if (!isOnCall) {
    title = `${label} unavailable: user must be On Call`;
  }

  return `<button class="supervisor-action-btn ${cssClass}" ${disabled ? "disabled" : ""} title="${title}" onclick="openSupervisorActionModal('${action}', '${agent.extension || ""}', '${agent.email || agent.display_name || "Unknown User"}')">${label}</button>`;
}

function openSupervisorActionModal(action, extension, userLabel) {
  if (!extension) return;
  ...
  currentSupervisorActionCode = `${fac}${extension}`;

  document.getElementById("supervisorActionTitle").textContent = `${actionLabel} - ${userLabel}`;
  document.getElementById("supervisorActionText").textContent =
    isWhisper
  ...
  document.getElementById("supervisorActionDialCode").textContent = currentSupervisorActionCode;
  document.getElementById("supervisorActionDialLink").href = `tel:${encodeURIComponent(currentSupervisorActionCode)}`;
  document.getElementById("supervisorActionModal").style.display = "flex";
}

function closeSupervisorActionModal() {
  document.getElementById("supervisorActionModal").style.display = "none";
}

async function copySupervisorActionCode() {
  try {
    await navigator.clipboard.writeText(currentSupervisorActionCode);
    alert(`Copied: ${currentSupervisorActionCode}`);
  } catch {
    alert(`Dial code: ${currentSupervisorActionCode}`);
  }
}
